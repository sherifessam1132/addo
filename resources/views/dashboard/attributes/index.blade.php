    ...
    <div class="card card-custom">
        <div class="card-header flex-wrap border-0 pt-6 pb-0">
            <div class="card-title">
                <h3 class="card-label">{{__('site.attribute.all')}}
{{--                    <div class="text-muted pt-2 font-size-sm">Datatable initialized from HTML table</div>--}}
                </h3>
            </div>
            <div class="card-toolbar">
                <!--begin::Button-->
                <a href="{{route('dashboard.attributes.create')}}" class="btn btn-primary font-weight-bolder">
            ...
            var _demo = function() {
                ...
                var datatable = $('#kt_datatable').KTDatatable({
                    ...
                    columns: [
                        ...
                        {
                            field: 'name',
                            title: '{{__('site.attribute.name')}}',
                            template: function(data) {
                                if (data.child == null){
                                    return data.name;
                                }
                                return `<a href="{{route('dashboard.attributes.index')}}/${data.id}">${data.name}</a>` ;
                            }
                        },
                        {
                            field: 'category.name',
                            sortable: false,
                            title: '{{__('site.attribute.category')}}',
                            template: function(data) {
                                let parent='';
                                if(data.category.parent!=null){
                                    parent=data.category.parent.name;
                                }
                                return  `${parent} / `+data.category.name   ;
                            }
                        },
                        {
                            field: 'Actions',
